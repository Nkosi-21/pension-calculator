...
def predict_benefit(model, gender, entry_age, exit_age, duration, salary, prev_bal, contrib, inv, inf):
    """
    No scaling needed — Stochastic GB is the winning model (tree-based, scale invariant)
    Features match Stage 1 and 2 exactly:
    Gender, EntryAge, ExitAge, Duration_FromAge,
    Annual Salary, PrevAccBal, Annual Contrib,
    InvestmentRate, InflationRate, RealReturn,
    PrevAcc_x_Duration, Contrib_x_Duration, InvestRate_x_Duration
    """
    real_return = inv - inf
    features = np.array([[
        gender,
